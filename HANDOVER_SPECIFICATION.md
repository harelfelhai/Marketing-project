...
**Carry-over tech debt for Phase F (do not address standalone):** the legacy tables `Entity` and `ActionLog` and every service that writes to them still use naive `datetime.utcnow()` (Python 3.12-deprecated). Phase DX introduced the timezone-aware pattern (`datetime.now(timezone.utc)` + `UTCDateTime` TypeDecorator, now in `backend/models/types.py`) for the new `PipelineTask` table; **Phase DY migrated `PhoneNumber`** to the same contract because the table was being touched for the new scoring columns and a half-migrated state would have created comparison hazards. The remaining `Entity` + `ActionLog` migration is bundled into Phase F because the Alembic SQLite → PostgreSQL cutover already touches every datetime column. Cross-table joins must remain tolerant of mixed tz-aware (PhoneNumber, PipelineTask) vs tz-naive (Entity, ActionLog) shapes until then.
