 ...
 * tooltips that open upward (bottom-full) so they escape any row height
 * constraint.
 *
 * In RTL, "last two" anchoring flips to left-0 so tooltips don't clip
 * the inline-start (right) edge.
 */
...
import { ACTION_PIPELINE_EMPTY } from '../../config/strings.he';
...
export default function ActionMiniPipeline({ logs }) {
  if (!logs || logs.length === 0) {
    return <span className="text-xs text-slate-400 italic">{ACTION_PIPELINE_EMPTY}</span>;
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      {slice.map((log, idx) => {
        ...
        // In RTL, anchor the tooltip for the last two icons to left-0 (inline-end)
        // to prevent them from overflowing the start (right) edge.
        const anchorLeft = idx >= slice.length - 2 && slice.length >= 3;
        const tooltipPosition = anchorLeft
          ? 'left-0'
            ...
            >
              <span className="block bg-slate-900 text-white text-[11px] leading-snug rounded-md px-2.5 py-1.5 shadow-lg">
                ...
                <span className="block text-slate-200">
                  {actionLabel(log.status)} · #{log.retry_count}
                </span>
