 ...
 *
 * Accepts an optional rootEntity prop (the primary entity for this client)
 * to display role and identifier fields.
 *
...
import { ArrowUpRight, ClipboardList, Phone as PhoneIcon } from 'lucide-react';
...
const _ALL_SECTIONS = new Set(['role', 'identifier', 'phone_count', 'metrics', 'verdicts', 'sla', 'tasks']);

export default function ClientCard({ client, rootEntity }) {
  ...
  const identifier = rootEntity?.identifier_1;
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* Role */}
        {visibleSections.has('role') && role && (
          <p className="text-xs text-slate-500 -mt-2 truncate" title={role}>{role}</p>
        )}

        {/* Identifier + phone count row */}
        {(visibleSections.has('identifier') || visibleSections.has('phone_count')) && (
          <div className="flex items-center gap-4 text-xs text-slate-600">
            {visibleSections.has('identifier') && identifier && (
              <span className="font-mono">{identifier}</span>
            )}
            {visibleSections.has('phone_count') && (
