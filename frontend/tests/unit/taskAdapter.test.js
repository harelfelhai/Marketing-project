 ...
 *
 * Verifies:
 *   - enrichTask attaches client_name from clientRegistry by integer client_id.
 *   - enrichTaskList runs enrichment over every item, including null-safety.
 *   - Unknown client_id values fall back to the registry's default mapping.
...
describe('enrichTask', () => {
  it('attaches client_name resolved from clientRegistry', () => {
    const task = {
      ...
      status: 'pending', requested_by: 'op', client_id: 1,
    };
  ...
  it('falls back to the registry default for unknown client_id', () => {
    const enriched = enrichTask({ id: 99, client_id: 9999 });
  ...
  it('handles null client_id without throwing', () => {
    const enriched = enrichTask({ id: 5, client_id: null });
...
describe('enrichTaskList', () => {
  it('enriches each item in the list', () => {
    ...
    enriched.forEach((t, i) => {
      expect(t.client_name).toBe(getClientName(items[i].client_id));
    });
  });
